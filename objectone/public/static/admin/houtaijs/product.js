...
//    序号被选中时，底部按钮显示
    ...
    $(".menubtn1").click(function(){
        ...
        $("#page1").show().siblings(".container-bottom-bottom").hide();
    });
    $(".menubtn2").click(function(){
        $("#tb_2").show().siblings(".tb").hide();
        $("#page2").show().siblings(".container-bottom-bottom").hide();
    });
    $(".menubtn3").click(function(){
        $("#tb_3").show().siblings(".tb").hide();
        $("#page3").show().siblings(".container-bottom-bottom").hide();
    });
    $(".menubtn4").click(function(){
         $("#tb_4").show().siblings(".tb").hide();
        $("#page4").show().siblings(".container-bottom-bottom").hide();
    });
    //当屏幕改变的时候
...
//    付款截图点击
    ...
    //3张付款截图点击
    $(".threephoto>.payphoto").click(function(){
        //获取弹出菜单的高度和宽度
        var clheight =$(".checklist").height();
        var clwidth = $(".checklist").width();
        if($(this).children(".screenshot").css("display")=="none"){
            $(".screenshot").height(clheight).width(clwidth + 2).show().children().show();
        }else{
            $(".screenshot").height(clheight).width(clwidth + 2).hide().children().hide();
        }
    ...
    $(".payphoto").on("click", function(e){
        $(this).siblings(".screenshot").show().children().show();
        $(document).one("click", function(){
            $(".screenshot").hide().children().hide();
        });
        e.stopPropagation();
...
//    客户资料，联系人点击添加class
    $(".correlationbtn1,.correlationbtn3,.correlationbtn5").click(function(){
        $(".correlationbtn6>div").text("编辑资料").parent();
    });
    $(".correlationbtn2").click(function(){
        $(".correlationbtn6>div").text("+ 联系人").parent();
    });
    $(".correlationbtn4").click(function(){
        $(".correlationbtn6>div").text("跟进筛选").parent();
    });
    $(".data").click(function(){
        if($(this).text()=="编辑资料"){
            //编辑资料操作
            alert(1);
        }else if($(this).text()=="跟进筛选"){
            //跟进筛选操作
            alert(2);
    ...
    $("#all").change(function(){
        var all = 0;
        $("table input[type='checkbox']:visible").each(function(i) {
            all++;
            $(".cheng").text(all);
        });
        if($(this).is(":checked")){
            $("table input[type='checkbox']:visible").prop("checked",true);
            $(".cheng").text(all);
        }else{
            $("table input[type='checkbox']:visible").prop("checked",false);
            $(".cheng").text("0");
        }
    });
//    序号全部选中时，底部全选被选中
    $("table input[type='checkbox']:visible").change(function(){
...
//         var audit ="";
//         audit += $(this).children("option:selected").text();
//     });
    var intext;       //存下拉菜单里的值
    var auditstr = "";
    var topstr="";
   ...
   /* //审核
    $(".auditbtn").click(function(){
        intext = $("#audit").children("option:selected").text();
        auditstr = '<span class="recommendcolor">['+intext+']</span>';
        if($("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").children().hasClass("auditcolor")){
            return false;
        }else{
            $("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").find("span:last-child").before(auditstr);
        }
    });*/
   /* //置顶
    $(".topbtn").click(function(){
        intext = $("#top").children("option:selected").text();
        topstr = '<span class="topcolor">['+intext+']</span>';
        if($("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").children().hasClass("topcolor")){
            return false;
        }else{
            $("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").find("span:last-child").before(topstr);
        }
    });*/
    //推荐
    /*$(".recommendbtn").click(function(){
        intexts = $("#recommend").children("option:selected").text();
        recommendstr = '<span class="recommendcolor">['+intexts+']</span>';
        if($("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").children().hasClass("recommendcolor")){
            return false;
        }else{
            $("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").find("span:last-child").before(recommendstr);
        }
    });*/
    //头条
    /*$(".topnewsbtn").click(function(){
        intext = $("#topnews").children("option:selected").text();
        topnewsstr = '<span class="topnewscolor">['+intext+']</span>';
        if($("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").children().hasClass("topnewscolor")){
            return false;
        }else{
            $("table:visible input[type='checkbox']:checked").parent("td").siblings(".rank").find("span:last-child").before(topnewsstr);
        }
    });*/
    //    表格里的tr点击的时候，前面选择框选中
